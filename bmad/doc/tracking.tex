...
Quadrupole tracking including support for misalignments, fringe fields (\vn{full} type including
hard and soft edge kicks), magnetic and electric multipole kicks, and the low-energy
$z$-correction. Split-step integration is used when multipoles are present, controlled by the
element's \vn{ds_step} parameter.
%
\item[\vn{Sextupole}] \Newline
Sextupole tracking using a drift-kick-drift split-step integrator. Supports magnetic and electric
multipoles of arbitrary order. The number of integration steps is controlled by \vn{ds_step}.
Misalignments are supported. Fringe fields fall back to CPU.
%
\item[\vn{Sbend}] \Newline
Sector bend tracking with support for the $k_1$ map, general nonlinear bend maps, magnetic and
electric multipoles with curvature corrections, misalignments (including curvature-aware
\vn{ref_tilt}), and the low-energy $z$-correction. Not supported when \vn{exact_multipoles} is set.
%
\item[\vn{Lcavity}] \Newline
Linac cavity tracking using a stair-step RF approximation. Supports both standing-wave and
traveling-wave cavities, including ponderomotive kicks for standing-wave cavities. Supports both
relative and absolute time tracking (\vn{absolute_time_tracking}), including correct
\vn{ref_time_start} subtraction for downstream cavities. Not supported when solenoid components are
present, multipoles are present, coupler kicks are non-zero, RF frequency is zero, or the element
has zero length.
%
\item[\vn{Pipe}] \Newline
Pipe element tracking (equivalent to drift body). Supports misalignments and apertures.
%
\item[\vn{Monitor}, \vn{Instrument}] \Newline
Monitor and instrument elements are tracked as pipe elements (drift body) on the GPU.
\end{description}
...
\label{s:gpu.radiation}

GPU tracking fully supports radiation damping (\vn{bmad_com%radiation_damping_on}) and radiation
fluctuations (\vn{bmad_com%radiation_fluctuations_on}). The stochastic and damping matrices from
Bmad's \vn{radiation_map_setup} are applied on the GPU using cuRAND for Gaussian random number
generation. The entrance and exit radiation kicks run as separate CUDA kernels within the same
device session, avoiding extra host-device transfers. Supported for all element types that produce
radiation (quadrupole, sbend, lcavity; drift and pipe do not radiate).

\subsection{Collective Effects}
\label{s:gpu.collective}

GPU tracking supports two collective effects:
\begin{description}
\item[\vn{3D FFT Space Charge}] (\vn{space_charge_method = fft_3d}) \Newline
The 3D Poisson solver uses cuFFT for the Green function convolution. Charge deposition (trilinear
with \vn{atomicAdd}), Green function computation, field interpolation, and kick application all run
as GPU kernels.
%
\item[\vn{1D CSR}] (\vn{csr_method = 1_dim}) \Newline
Particle binning and kick application run on the GPU. The bin-level kick calculation (involving
iterative root-finding for retarded-time integrals) remains on the CPU.
\end{description}
...
Rectangular and elliptical apertures are checked on the GPU as part of the persistent device session
(\sref{s:gpu.persistent}). Elements with other aperture types (wall3d, custom) fall back to
CPU-side aperture checking. The \vn{aperture_at} attribute is respected: entrance-only, exit-only,
and both-ends configurations are handled correctly.

\subsection{Persistent GPU Session}
\label{s:gpu.persistent}

Particle data is kept on the GPU across consecutive elements to minimize host-device transfers. On
the first eligible element, particles are uploaded to the GPU. Subsequent elements are tracked
entirely on device (misalignment, fringe, radiation, body kernel, aperture check) with no transfers.
Data is downloaded to the CPU only when a non-GPU element is encountered or tracking completes.

For a lattice with $N$ GPU-eligible elements in a row, this reduces the number of PCIe transfers
from $2N$ (one upload and one download per element) to 2 (one upload at the start, one download at
the end).

\subsection{Deferred Flush}
\label{s:gpu.deferred.flush}

The \vn{bmad_com%gpu_deferred_flush} flag (default \vn{False}) controls whether \vn{track_bunch}
downloads particle data from the GPU after each call. When \vn{True}, the download is deferred and
the caller is responsible for flushing at appropriate points.

Tao automatically enables this during beam tracking (\vn{tao_beam_track}), flushing only at save
points (start element, end element, and elements with \vn{save_beam_internally} set). Per-element
sigma matrix computation and lost-particle counting are also skipped at intermediate elements. This
eliminates the dominant overhead in Tao's tracking loop when GPU tracking is enabled.
