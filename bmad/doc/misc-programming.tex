...
a bunch are tracked in parallel by CUDA threads. Supported elements include drift, quadrupole
(with fringe), sbend (with misalignment), lcavity (relative and absolute time), pipe, monitor, and
instrument. Radiation damping/fluctuations, 3D FFT space charge, and 1D CSR are all supported on
GPU.

A persistent GPU session keeps particle data on the device across consecutive elements, reducing
host-device transfers from $O(N_\text{elements})$ to $O(1)$. Combined with deferred flush
(\vn{bmad_com%gpu_deferred_flush}), this gives typical speedups of 10--30$\times$ depending on
lattice size and particle count.
